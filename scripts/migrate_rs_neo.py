...
class Neo4jArticleReliability:
    ...
    def _update_reliability_scores(tx):
        ...
        for record in result_list:
            ...
            print(f"Article {article_node['id']} calculated reliability score: {reliability_index}")
            ...
            RETURN a.reliability_score AS reliability_score
            """
            check_result = tx.run(check_query, id=article_node["id"])
            for check_record in check_result:
                print(f"Updated reliability score for article {article_node['id']}: {check_record['reliability_score']}")
