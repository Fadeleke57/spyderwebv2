...
def get_items_by_field(
    collection_name: str, field: str, value: Any, ascending: bool = True
) -> List[Any]:
    collection = get_collection(collection_name)
...
def get_items_sorted_by_field(
    collection_name: str, field: str, ascending=True
) -> List[Dict[str, Any]]:
    collection = get_collection(collection_name)
...
def update_item_by_id(
    collection_name: str, item_id: str, update_data: Dict[str, Any]
) -> Dict[str, Any]:
    collection = get_collection(collection_name)
...
def replace_item_by_id(collection_name, item_id, new_data):
    ...
    return collection.find_one_and_replace(
        {"_id": ObjectId(item_id)}, new_data, return_document=ReturnDocument.AFTER
    )
...
def add_search_to_user(email, search_data):
    ...
    update_data = {
        "$push": {"analytics.searches": {"$each": [search_data], "$position": 0}}
    }

    return collection.find_one_and_update(
        {"email": email}, update_data, return_document=ReturnDocument.AFTER
    )
...
def clear_search_history(email):
    ...
    update_data = {"$set": {"analytics.searches": []}}
