...
from src.models.index import Source, Connection, create_process, update_process, Webs
...
class SourceService:
    ...
    def refresh_note_embeddings(self, source: Source, content: str):
        try:
            ...
            if not all_ids:
                logger.info(
                    f"No vectors found for this web: {source['webId']}. Nothing to update"
                )
                return True
            ...
            for i in range(0, len(all_ids), batch_size):
                ...
                fetch_response = pineconeClient.index.fetch(
                    ids=batch_ids, namespace=source["webId"]
                )
    ...
    def delete_source_embeddings(self, source: Source):
        try:
            ...
            for i in range(0, len(all_ids), batch_size):
                ...
                fetch_response = pineconeClient.index.fetch(
                    ids=batch_ids, namespace=source["webId"]
                )  # update to query with metadata filtering in the future for faster fetches
    ...
    def refresh_metadata(self, webId: str, sourceId: str, metadata: dict):
        ...
        try:
            ...
            if not all_ids:
                logger.info(
                    f"No vectors found for this web: {webId}. Nothing to update"
                )
                return True
    ...
    def create_onboarding_sources(self, web_id: str, user_id: str, background_tasks: BackgroundTasks):
        ...
        sources_to_insert = [source_to_insert1, source_to_insert2, source_to_insert3, source_to_insert4, source_to_insert5, source_to_insert6]
        neo4jClient.create_many_nodes("source", sources_to_insert)
        for source in sources_to_insert:
            if source["type"] ==  "website":
                background_tasks.add_task(self.embed_and_upsert_website, source=source, md=source["content"])
            elif source["type"] ==  "note":
                background_tasks.add_task(self.embed_and_upsert_note, source=source, text=source["content"])
