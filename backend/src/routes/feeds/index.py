...
def revoke_feed_access(
    ...
    if not feed:
        logger.error(f"Feed not found for client id {client_id} and user id {user_id}")
    ...
    logger.info(f"Revoked feed access for client id {client_id} and user id {user_id}")
